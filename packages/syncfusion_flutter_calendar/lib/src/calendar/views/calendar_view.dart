...
class _CustomCalendarScrollViewState extends State<CustomCalendarScrollView>
  ...
  Widget build(BuildContext context) {
    ...
    final bool isHorizontalNavigation = widget.calendar.monthViewSettings.navigationDirection ==
    ...
    final Widget customScrollWidget = GestureDetector(
      ...
      onHorizontalDragStart: isTimelineView
          ...
          : (DragStartDetails dragStartDetails) {
              _onHorizontalStart(dragStartDetails, isResourceEnabled, isTimelineView,
                  viewHeaderHeight, timeLabelWidth, isNeedDragAndDrop);
            },
      ...
      onVerticalDragStart: isHorizontalNavigation
          ? null
          : (DragStartDetails dragStartDetails) {
              _onVerticalStart(dragStartDetails, isResourceEnabled, isTimelineView,
                  viewHeaderHeight, timeLabelWidth, isNeedDragAndDrop);
            },
      ...
      onVerticalDragEnd: isHorizontalNavigation
          ? null
          : (DragEndDetails dragEndDetails) {
              _onVerticalEnd(dragEndDetails, isResourceEnabled, isTimelineView, isMonthView,
                  viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth, isNeedDragAndDrop);
            },
    ...
    return GestureDetector(
      onLongPressStart: (LongPressStartDetails details) {
        _handleLongPressStart(details, isNeedDragAndDrop, isTimelineView, isResourceEnabled,
      ...
      onLongPressEnd: isNeedDragAndDrop
          ? (LongPressEndDetails details) {
              _handleLongPressEnd(details.localPosition, isTimelineView, isResourceEnabled,
                  isMonthView, viewHeaderHeight, timeLabelWidth, weekNumberPanelWidth);
            }
          : null,
      child: Stack(
        children: <Widget>[
  ...
  void _handleAppointmentDragStart(AppointmentView appointmentView, bool isTimelineView,
      Offset details, bool isResourceEnabled, double viewHeaderHeight, double timeLabelWidth) {
    ...
    if (isTimelineView) {
      xPosition =
          (appointmentPosition.dx - currentState._scrollController!.position.pixels) - details.dx;
      if (widget.isRTL) {
    ...
    if (widget.calendar.onDragStart != null) {
      widget.calendar
          .onDragStart!(AppointmentDragStartDetails(dragStartAppointment, selectedResource));
    }
  }

  void _handleLongPressStart(LongPressStartDetails details, bool isNeedDragAndDrop,
      bool isTimelineView, bool isResourceEnabled, double viewHeaderHeight, double timeLabelWidth) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    ...
    _handleAppointmentDragStart(appointmentView, isTimelineView, details.localPosition,
  ...
  Future<void> _updateNavigationDayView(
    ...
    if (widget.calendar.dragAndDropSettings.allowScroll &&
      ...
      _timer = Timer.periodic(widget.calendar.dragAndDropSettings.autoNavigateDelay,
          (Timer timer) async {
        if (_dragDetails.value.position.value != null &&
            ((isHorizontalNavigation &&
        ...
        ((isHorizontalNavigation &&
                (appointmentPosition.dx + navigationThresholdValue) - rtlValue <= 0) ||
      ...
      _timer = Timer.periodic(const Duration(seconds: 1), (Timer timer) async {
        if (_dragDetails.value.position.value != null &&
            ((isHorizontalNavigation &&
                    (_dragDetails.value.position.value!.dx + navigationThresholdValue) - rtlValue <=
                        0) ||
                (!isHorizontalNavigation &&
                    _dragDetails.value.position.value!.dy <= viewHeaderHeight))) {
          if (widget.isRTL) {
            _moveToNextViewWithAnimation();
          } else {
            _moveToPreviousViewWithAnimation();
        ...
        _timer = Timer(const Duration(milliseconds: 200), () async {
          if (_dragDetails.value.position.value != null &&
            ...
            Future<void> updateScrollPosition() async {
              ...
              if (_dragDetails.value.position.value != null &&
                  _dragDetails.value.position.value!.dy - viewHeaderHeight - timeIntervalHeight <=
                      0 &&
                  currentState._timelineViewVerticalScrollController!.position.pixels != 0) {
                updateScrollPosition();
              } else if (_timer != null) {
  ...
  void _updateAppointmentDragUpdateCallback(
    ...
    if (!isMonthView) {
      if (isTimelineView) {
        final DateTime time = _timeFromPosition(draggingTime, widget.calendar.timeSlotViewSettings,
            xPosition, currentState, timeIntervalHeight, isTimelineView)!;

        draggingTime = DateTime(
            draggingTime.year, draggingTime.month, draggingTime.day, time.hour, time.minute);
      } else {
        if (yPosition < 0) {
          draggingTime = DateTime(draggingTime.year, draggingTime.month, draggingTime.day);
        } else {
          draggingTime = _timeFromPosition(draggingTime, widget.calendar.timeSlotViewSettings,
              yPosition, currentState, timeIntervalHeight, isTimelineView)!;
        }
      }
    }
  ...
  void _handleLongPressEnd(
    ...
    if (!isMonthView) {
      if (isTimelineView) {
        final DateTime time = _timeFromPosition(dropTime!, widget.calendar.timeSlotViewSettings,
            xPosition, currentState, timeIntervalHeight, isTimelineView)!;
        dropTime = DateTime(dropTime.year, dropTime.month, dropTime.day, time.hour, time.minute);
      } else {
        dropTime = _timeFromPosition(dropTime!, widget.calendar.timeSlotViewSettings, yPosition,
    ...
    DateTime updateStartTime =
        isAllDay ? DateTime(dropTime!.year, dropTime.month, dropTime.day) : dropTime!;
    ...
    if ((appointment.recurrenceRule != null && appointment.recurrenceRule!.isNotEmpty) ||
      ...
      final List<DateTime> recurrenceDates = RecurrenceHelper.getRecurrenceDateTimeCollection(
          ...
          specificEndDate:
              currentState.widget.visibleDates[currentState.widget.visibleDates.length - 1]);
      ...
      if (!CalendarViewHelper.isDateInDateCollection(recurrenceDates, callbackStartDate)) {
        ...
        if (currentRecurrenceIndex == 0 || currentRecurrenceIndex == recurrenceDates.length - 1) {
          ...
          canAddRecurrence =
              (isDateWithInDateRange(previousRecurrence, nextRecurrence, callbackStartDate) &&
    ...
    widget.calendar.dataSource!
        .notifyListeners(CalendarDataSourceAction.add, <dynamic>[newAppointment]);

    _resetDraggingDetails(currentState);
    if (widget.calendar.onDragEnd != null) {
      widget.calendar.onDragEnd!(AppointmentDragEndDetails(
          newAppointment, previousResource, selectedResource, callbackStartDate));
    }
  }
  ...
  void _updateVisibleDates() {
    ...
    _visibleDates = getVisibleDates(
            currentDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
  ...
  void _updateNextViewVisibleDates() {
    ...
    List<DateTime> dates = getVisibleDates(
            currentViewDate, nonWorkingDays, widget.calendar.firstDayOfWeek, visibleDatesCount)
        .cast();
  ...
  void getRecurrenceRegions(CalendarTimeRegion region, List<CalendarTimeRegion> regions,
    ...
    final List<DateTime> recursiveDates = RecurrenceHelper.getRecurrenceDateTimeCollection(
        ...
        recurrenceDuration:
            AppointmentHelper.getDifference(region.actualStartTime, region.actualEndTime),
  ...
  _CalendarView _updateViews(
    ...
    else if (!CalendarViewHelper.isCollectionEqual(appointmentLayout.visibleAppointments.value,
  ...
  void _updateDayViewScrollPosition() {
    ...
    if (_previousViewKey.currentState!._scrollController!.offset != scrolledPosition &&
        _previousViewKey.currentState!._scrollController!.position.maxScrollExtent >=
            scrolledPosition) {
    ...
    if (_currentViewKey.currentState!._scrollController!.offset != scrolledPosition &&
        _currentViewKey.currentState!._scrollController!.position.maxScrollExtent >=
            scrolledPosition) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if ((rowIndex * singleChildWidth) + xPosition + currentViewState._timeIntervalHeight >=
        ...
        currentViewState._scrollController!.jumpTo(
            currentViewState._scrollController!.offset + currentViewState._timeIntervalHeight);
      }
  ...
  DateTime? _updateSelectedDateForUpArrow(
    ...
    if (widget.view == CalendarView.month) {
      ...
      selectedDate = AppointmentHelper.addDaysWithTime(selectedDate, -DateTime.daysPerWeek,
        ...
        if (CalendarViewHelper.isDateInDateCollection(currentView.blackoutDates, selectedDate)) {
          do {
            selectedDate = AppointmentHelper.addDaysWithTime(
              ...
              CalendarViewHelper.isDateInDateCollection(currentView.blackoutDates, selectedDate));
        }
      }
  ...
  DateTime? _updateSelectedDateForDownArrow(
    ...
    if (widget.view == CalendarView.month) {
      ...
      if (selectedDate.add(widget.calendar.timeSlotViewSettings.timeInterval).day !=
          selectedDate.day) {
        return selectedDate;
      }
  ...
  void _moveToSelectedTimeSlot() {
    ...
    final double scrollPosition = currentViewState
        ._getScrollPositionForCurrentDate(currentViewState._selectionPainter!.selectedDate!);
    if (scrollPosition == -1 ||
  ...
  DateTime? _updateSelectedDate(
    ...
    if (event.logicalKey == LogicalKeyboardKey.arrowRight) {
      do {
        selectedDate =
            _updateSelectedDateForRightArrow(currentView, currentViewState, selectedDate);
  ...
  bool _isSelectedDateEnabled(DateTime date, int resourceIndex, [bool isMinMaxDate = false]) {
    ...
    final int timeInterval =
        CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings);
    if ((isMonthView &&
            !isDateWithInDateRange(widget.calendar.minDate, widget.calendar.maxDate, date)) ||
        (!isMonthView &&
  ...
  KeyEventResult _updatePageUpAndDown(
      KeyEvent event, _CalendarViewState currentViewState, bool isResourceEnabled) {
    ...
    if (HardwareKeyboard.instance.isShiftPressed) {
      if (event.logicalKey == LogicalKeyboardKey.tab) {
        if (currentAllDayAppointment != null || currentSelectedAppointment != null) {
          int index = tempAppColl
              .indexOf(isAllDay ? currentAllDayAppointment : currentSelectedAppointment!);
          index -= 1;
          if (tempAppColl.length > index && !index.isNegative) {
            selectedAppointment =
                tempAppColl[index].appointment != null ? tempAppColl[index] : null;
          }
        }
        ...
        return _updateAppointmentSelectionOnView(selectedAppointment, currentVisibleViewState,
            isAllDay, isResourceEnabled, !HardwareKeyboard.instance.isShiftPressed);
      }
    } else if (event.logicalKey == LogicalKeyboardKey.tab) {
      if (currentAllDayAppointment != null || currentSelectedAppointment != null) {
        int index =
            tempAppColl.indexOf(isAllDay ? currentAllDayAppointment : currentSelectedAppointment!);
        index += 1;
  ...
  KeyEventResult _updateAppointmentSelectionOnView(
    ...
    final DateTime visibleEndDate = AppointmentHelper.convertToEndTime(currentVisibleViewState
    ...
    if (selectedAppointment != null &&
      ...
      if (widget.controller.view != CalendarView.month) {
        ...
        final double scrollViewHeight =
            currentVisibleViewState._scrollController!.position.maxScrollExtent +
        ...
        if (isTimeline) {
          ...
          if (isDayView) {
            ...
            allDayHeight =
                allDayHeight > _kAllDayLayoutHeight ? _kAllDayLayoutHeight : allDayHeight;
          }
  ...
  KeyEventResult _onKeyDown(FocusNode node, KeyEvent event) {
    ...
    if (event.logicalKey == LogicalKeyboardKey.enter &&
      ...
      CalendarViewHelper.raiseCalendarTapCallback(
          ...
          isResourcesEnabled
              ? widget
                  .calendar.dataSource!.resources![currentVisibleViewState._selectedResourceIndex]
              : null);
    }
    ...
    if ((currentVisibleViewState._selectionPainter!.selectedDate != null &&
      ...
      final DateTime? selectedDate = _updateSelectedDate(event, currentVisibleViewState,
          currentVisibleView, resourceIndex, selectedAppointmentDate, isAllDayAppointment);
  ...
  void _positionTimelineView({bool isScrolledToEnd = true}) {
    ...
    if (widget.isRTL) {
      if (_currentChildIndex == 0) {
        currentViewState._scrollController!.jumpTo(
            isScrolledToEnd ? currentViewState._scrollController!.position.maxScrollExtent : 0);
        nextViewState._scrollController!.jumpTo(0);
      } else if (_currentChildIndex == 1) {
        nextViewState._scrollController!.jumpTo(
            isScrolledToEnd ? nextViewState._scrollController!.position.maxScrollExtent : 0);
        previousViewState._scrollController!.jumpTo(0);
      } else if (_currentChildIndex == 2) {
        previousViewState._scrollController!.jumpTo(
            isScrolledToEnd ? previousViewState._scrollController!.position.maxScrollExtent : 0);
  ...
  void _onHorizontalStart(DragStartDetails dragStartDetails, bool isResourceEnabled,
      bool isTimelineView, double viewHeaderHeight, double timeLabelWidth, bool isNeedDragAndDrop) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    if (currentState._hoveringAppointmentView != null &&
      ...
      _handleAppointmentDragStart(
          ...
          isTimelineView,
          Offset(
              dragStartDetails.localPosition.dx - widget.width, dragStartDetails.localPosition.dy),
          isResourceEnabled,
          viewHeaderHeight,
          timeLabelWidth);
      return;
  ...
  void _onVerticalStart(DragStartDetails dragStartDetails, bool isResourceEnabled,
      bool isTimelineView, double viewHeaderHeight, double timeLabelWidth, bool isNeedDragAndDrop) {
    final _CalendarViewState currentState = _getCurrentViewByVisibleDates()!;
    if (currentState._hoveringAppointmentView != null &&
        currentState._hoveringAppointmentView!.appointment != null &&
      ...
      _handleAppointmentDragStart(
          ...
          isTimelineView,
          Offset(
              dragStartDetails.localPosition.dx, dragStartDetails.localPosition.dy - widget.height),
          isResourceEnabled,
          viewHeaderHeight,
          timeLabelWidth);
      return;
  ...
  void _updateAppointmentPainter() {
    for (int i = 0; i < _children.length; i++) {
      ...
      if (widget.view == CalendarView.month && widget.calendar.monthCellBuilder != null) {
        ...
        if (view.visibleDates == _currentViewVisibleDates) {
          widget.getCalendarState(_updateCalendarStateDetails);
          if (!CalendarViewHelper.isCollectionEqual(appointmentLayout.visibleAppointments.value,
              _updateCalendarStateDetails.visibleAppointments)) {
            appointmentLayout.visibleAppointments.value =
                _updateCalendarStateDetails.visibleAppointments;
          }
...
class _CalendarViewState extends State<_CalendarView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width,
    ...
    _currentTimeNotifier =
        ValueNotifier<int>((today.day * 24 * 60) + (today.hour * 60) + today.minute);
    _timer = _createTimer();
    super.initState();
  }
  ...
  void didUpdateWidget(_CalendarView oldWidget) {
    ...
    final bool isCurrentView =
        _updateCalendarStateDetails.currentViewVisibleDates == widget.visibleDates;
        ...
        children: <Widget>[
          GestureDetector(
            ...
            child: SizedBox(
                height: widget.height,
                width: widget.width,
                child: _addDayView(widget.width, _timeIntervalHeight * _horizontalLinesCount!,
                    _isRTL, widget.locale, isCurrentView)),
          ),
          _getResizeShadowView()
        ],
      ),
  ...
  void _scrollToPosition() {
    SchedulerBinding.instance.addPostFrameCallback((_) {
      ...
      if (scrollPosition == -1 ||
          (_scrollController != null && _scrollController!.position.pixels == scrollPosition)) {
        return;
      }
      if (_scrollController != null) {
  ...
  double _getScrollPositionForCurrentDate(DateTime date) {
    ...
    if (!isDateWithInDateRange(
        widget.visibleDates[0], widget.visibleDates[visibleDatesCount - 1], date)) {
      return -1;
    }
  ...
  void _retainScrolledDateTime() {
    ...
    final double time =
        ((CalendarViewHelper.getTimeInterval(widget.calendar.timeSlotViewSettings) / 60) *
                columnIndex) +
  ...
  double _getPositionFromDate(DateTime date) {
    final int visibleDatesCount = widget.visibleDates.length;
    _timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view, widget.width,
        widget.height, visibleDatesCount, widget.isMobilePlatform);
    double timeToPosition = 0;
    final bool isTimelineView = CalendarViewHelper.isTimelineView(widget.view);
    if (!isTimelineView) {
      timeToPosition = AppointmentHelper.timeToPosition(widget.calendar, date, _timeIntervalHeight);
    ...
    if (!isTimelineView) {
      final double scrollViewHeight = widget.height -
          _allDayHeight -
          CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view);
      final double scrollViewContentHeight = CalendarViewHelper.getHorizontalLinesCount(
              widget.calendar.timeSlotViewSettings, widget.view) *
          _timeIntervalHeight;
      maxScrollPosition = scrollViewContentHeight - scrollViewHeight;
    } else {
      final double scrollViewContentWidth = CalendarViewHelper.getHorizontalLinesCount(
              widget.calendar.timeSlotViewSettings, widget.view) *
          _timeIntervalHeight *
          visibleDatesCount;
  ...
  Widget _addAllDayAppointmentPanel(SfCalendarThemeData calendarTheme, bool isCurrentView) {
    ...
    double panelHeight =
        isCurrentView ? _updateCalendarStateDetails.allDayPanelHeight - _allDayHeight : 0;
    if (panelHeight < 0) {
      panelHeight = 0;
    }
    ...
    return Positioned(
      ...
      child: Stack(
        children: <Widget>[
          ...
          Positioned(
              left: 0, top: allDayExpanderHeight - 1, right: 0, height: 1, child: shadowView),
        ],
      ),
    );
  }
  ...
  Widget _getAllDayLayout(
    ...
    final Widget allDayLayout = AllDayAppointmentLayout(
        ...
        widget.isMobilePlatform,
        widget.width,
        (isDayView && _updateCalendarStateDetails.allDayPanelHeight < _allDayHeight) ||
                !isCurrentView
            ? _allDayHeight
            : _updateCalendarStateDetails.allDayPanelHeight,
        widget.localizations,
  ...
  void _onVerticalUpdate(DragUpdateDetails details) {
    ...
    if (!CalendarViewHelper.isTimelineView(widget.view) && widget.view != CalendarView.month) {
      ...
      _updateAutoScrollDay(details, viewHeaderHeight, allDayPanelHeight, isForwardResize,
          isBackwardResize, yPosition);
    }
  ...
  void _onVerticalEnd(DragEndDetails details) {
    ...
    double yPosition =
        currentYPosition - viewHeaderHeight - allDayPanelHeight + _scrollController!.offset;

    final CalendarAppointment appointment = _resizingDetails.value.appointmentView!.appointment!;
    final double timeIntervalHeight = _getTimeIntervalHeight(widget.calendar, widget.view,
        widget.width, widget.height, widget.visibleDates.length, widget.isMobilePlatform);
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
      ...
      if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(
        ...
        CalendarViewHelper.raiseAppointmentResizeEndCallback(widget.calendar, appointment.data,
            null, appointment.exactStartTime, appointment.exactEndTime);
      }
    ...
    appointment.recurrenceId =
        parentAppointment != null ? parentAppointment.id : appointment.recurrenceId;
  ...
  void _onHorizontalUpdate(DragUpdateDetails details) {
    ...
    if (isTimelineView) {
      ...
      _updateAutoScrollTimeline(details, timeIntervalHeight, isForwardResize, isBackwardResize,
          xPosition, yPosition, timeLabelWidth, isResourceEnabled);
    } else if (widget.view == CalendarView.month) {
      final double viewHeaderHeight =
          CalendarViewHelper.getViewHeaderHeight(widget.calendar.viewHeaderHeight, widget.view);
  ...
  void _onHorizontalEnd(DragEndDetails details) {
    ...
    if (_resizingDetails.value.isAllDayPanel ||
      ...
      final DateTime time = _timeFromPosition(resizingTime, widget.calendar.timeSlotViewSettings,
          xPosition, this, timeIntervalHeight, isTimelineView)!;

      resizingTime = DateTime(resizingTime.year, resizingTime.month, resizingTime.day, time.hour,
    ...
    final bool isMonthView =
        widget.view == CalendarView.timelineMonth || widget.view == CalendarView.month;
    ...
    if (CalendarViewHelper.isDraggingAppointmentHasDisabledCell(
      ...
      if (CalendarViewHelper.shouldRaiseAppointmentResizeEndCallback(
          widget.calendar.onAppointmentResizeEnd)) {
        CalendarViewHelper.raiseAppointmentResizeEndCallback(widget.calendar, appointment.data,
            resource, appointment.exactStartTime, appointment.exactEndTime);
      }
        ...
        if (yPosition != null &&
          ...
          Future<void> updateScrollPosition() async {
            ...
            _resizingDetails.value.position.value = Offset(
                _resizingDetails.value.appointmentView!.appointmentRect!.left, yPosition! - 0.1);
      ...
      case CalendarView.workWeek:
        {
          if (_resizingDetails.value.isAllDayPanel) {
            ...
            final double timeIntervalSize = _getTimeIntervalHeight(widget.calendar, widget.view,
                widget.width, widget.height, widget.visibleDates.length, widget.isMobilePlatform);
      ...
      case CalendarView.month:
        {
          final double weekNumberPanelWidth = CalendarViewHelper.getWeekNumberPanelWidth(
              widget.calendar.showWeekNumber, widget.width, widget.isMobilePlatform);
          final double minimumCellWidth =
              ((widget.width - weekNumberPanelWidth) / DateTime.daysPerWeek) / 2;
          if (isForwardResize) {
            _maximumResizingPosition = appointmentView.appointmentRect!.left +
  ...
  void _updateAppointmentResizingUpdateCallback(bool isForwardResize, bool isBackwardResize,
      double yPosition, double? viewHeaderHeight, double? allDayPanelHeight,
      {bool isResourceEnabled = false,
      double? timeLabelWidth,
      double? xPosition,
      DragUpdateDetails? details}) {
    ...
    if (CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      resizingTime =
          _getDateFromPosition(updatedXPosition, details.localPosition.dy, timeLabelWidth!)!;
  ...
  Widget _getResizeShadowView() {
    ...
    final bool isAllDayPanel =
        !isVerticalResize && (!isTimelineView && widget.view != CalendarView.month);
    final double timeLabelWidth = CalendarViewHelper.getTimeLabelWidth(
        widget.calendar.timeSlotViewSettings.timeRulerSize, widget.view);
    final double weekNumberPanelWidth = CalendarViewHelper.getWeekNumberPanelWidth(
        widget.calendar.showWeekNumber, widget.width, widget.isMobilePlatform);
  ...
  void _updateProgrammaticSelectedResourceIndex() {
    if (_updateCalendarStateDetails.selectedDate != null && _selectedResourceIndex == -1) {
      ...
      if ((isTimelineMonth &&
          ...
          (!isTimelineMonth &&
              (CalendarViewHelper.isSameTimeSlot(_updateCalendarStateDetails.selectedDate,
                  widget.calendar.initialSelectedDate)))) {
        _selectedResourceIndex = 0;
      }
    }
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (!widget.isMobilePlatform &&
          widget.calendar.monthViewSettings.appointmentDisplayMode ==
              MonthAppointmentDisplayMode.appointment) {
        appointmentView =
            _appointmentLayout.getAppointmentViewOnPoint(xDetails, yDetails - viewHeaderHeight);
        isMoreTapped = appointmentView != null &&
            appointmentView.startIndex == -1 &&
            appointmentView.endIndex == -1 &&
            appointmentView.position == -1 &&
  ...
  CalendarDetails? _getDetailsForTimeline(Offset position) {
    ...
    if (CalendarViewHelper.isResourceEnabled(widget.calendar.dataSource, widget.view)) {
      ...
      _selectedResourceIndex =
          _getSelectedResourceIndex(yPosition, viewHeaderHeight, timeLabelHeight);
      calendarResource = widget.calendar.dataSource!.resources![_selectedResourceIndex];
    }

    if (_isRTL) {
      xPosition =
          _scrollController!.offset + (_scrollController!.position.viewportDimension - xDetails);
      xPosition = (_scrollController!.position.viewportDimension +
              _scrollController!.position.maxScrollExtent) -
          xPosition;
    }
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (canRaiseLongPress || canRaiseTap || canRaiseSelectionChanged) {
        ...
        if (appointmentView == null) {
          ...
          if (canRaiseTap) {
            CalendarViewHelper.raiseCalendarTapCallback(widget.calendar, selectedDate, null,
                CalendarElement.calendarCell, selectedResource);
          } else if (canRaiseLongPress) {
            CalendarViewHelper.raiseCalendarLongPressCallback(widget.calendar, selectedDate, null,
                CalendarElement.calendarCell, selectedResource);
          }
          _updatedSelectionChangedCallback(canRaiseSelectionChanged, previousSelectedDate,
              selectedResource, previousSelectedResourceIndex);
        } else {
          if (canRaiseTap) {
            CalendarViewHelper.raiseCalendarTapCallback(
                widget.calendar,
  ...
  CalendarDetails? _getDetailsForDay(Offset position) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (isDayView &&
        ...
        return CalendarDetails(null, _getTappedViewHeaderDate(position, widget.width),
            CalendarElement.viewHeader, null);
      ...
      if (isTappedOnCount && selectedDate != null) {
        ...
        if (currentSelectedIndex != -1) {
          ...
          for (int i = 0;
              i < _updateCalendarStateDetails.allDayAppointmentViewCollection.length;
              i++) {
    ...
    if (yDetails < viewHeaderHeight) {
      ...
      if (appointmentView == null) {
        if (_isRTL) {
          ...
          _drawSelection(xDetails - timeLabelWidth, yDetails - viewHeaderHeight - allDayHeight,
              timeLabelWidth);
        }
      ...
      if (_selectionPainter!.selectedDate != null && targetElement != CalendarElement.allDayPanel) {
        ...
        if (canRaiseTap) {
          CalendarViewHelper.raiseCalendarTapCallback(widget.calendar,
              _selectionPainter!.selectedDate, selectedAppointments, targetElement, null);
        } else if (canRaiseLongPress) {
  ...
  bool _isEnabledRegion(double y, DateTime? selectedDate, int resourceIndex) {
    ...
    if (CalendarViewHelper.isTimelineView(widget.view)) {
      ...
      if (_isRTL) {
        minutes =
            ((_scrollController!.offset + (_scrollController!.position.viewportDimension - y)) %
                    viewWidth) ~/
                minuteHeight;
      } else {
    ...
    final DateTime date = DateTime(selectedDate.year, selectedDate.month, selectedDate.day, 0,
  ...
  void _updateHoveringForViewHeader(
    ...
    final DateTime? hoverDate = _getTappedViewHeaderDate(
        Offset(CalendarViewHelper.isTimelineView(widget.view) ? localPosition.dx : xPosition,
            yPosition),
        widget.width);
  ...
  void _updateMouseCursorForAppointment(
    ...
    if (isAllDayPanel || (widget.view == CalendarView.month || isTimelineViews)) {
      ...
      final bool canAddForwardSpanIcon = AppointmentHelper.canAddForwardSpanIcon(
          appStartTime, appEndTime, viewStartDate, viewEndDate);
      final bool canAddBackwardSpanIcon = AppointmentHelper.canAddBackwardSpanIcon(
          appStartTime, appEndTime, viewStartDate, viewEndDate);
      ...
      if (yPosition >= appointmentView.appointmentRect!.top &&
          ...
          CalendarViewHelper.isSameTimeSlot(appointmentView.appointment!.actualStartTime,
              appointmentView.appointment!.exactStartTime)) {
        setState(() {
          ...
          CalendarViewHelper.isSameTimeSlot(appointmentView.appointment!.actualEndTime,
              appointmentView.appointment!.exactEndTime)) {
        setState(() {
          _mouseCursor = SystemMouseCursors.resizeDown;
        });
  ...
  void _updatePointerHover(Offset globalPosition) {
    ...
    if (widget.view != CalendarView.month && !isTimelineViews) {
      ...
      if (localPosition.dy < viewHeaderHeight) {
        if (isDayView) {
          if ((_isRTL && localPosition.dx < widget.width - timeLabelWidth) ||
            ...
            _updateMouseCursorForAppointment(
                appointment, localPosition.dx, localPosition.dy, isTimelineViews,
                isAllDayPanel: true);
            return;
          }
        ...
        _updateHoveringForViewHeader(
            localPosition, localPosition.dx, localPosition.dy, viewHeaderHeight);
        return;
      }
    ...
    final DateTime? hoverDate = _getDateFromPosition(
        isTimelineViews ? localPosition.dx : xPosition, yPosition, timeLabelWidth);
    ...
    if (((widget.view == CalendarView.day ||
        ...
        (isTimelineViews &&
            !_isEnabledRegion(localPosition.dx, hoverDate, hoveringResourceIndex))) {
      if (_hoveringDate != null) {
        _hoveringDate = null;
      }
  ...
  DateTime? _getDateFromPositionForTimeline(
    ...
    if (_isRTL) {
      ...
      rowIndex =
          (((_scrollController!.offset % _getSingleViewWidthForTimeLineView(this)) + x) / cellWidth)
              .truncate();
    }
...
class _ViewHeaderViewPainter extends CustomPainter {
  ...
  void _addViewHeaderForMonthView(Canvas canvas, Size size, TextStyle viewHeaderDayStyle,
      double width, DateTime today, double weekNumberPanelWidth) {
    ...
    bool hasToday =
        monthViewSettings.numberOfWeeksInView > 0 && monthViewSettings.numberOfWeeksInView < 6 ||
            visibleDates[visibleDatesLength ~/ 2].month == today.month;
    if (hasToday) {
      hasToday =
          isDateWithInDateRange(visibleDates[0], visibleDates[visibleDatesLength - 1], today);
    }

    for (int i = 0; i < DateTime.daysPerWeek; i++) {
      final DateTime currentDate = visibleDates[i];
      String dayText =
          DateFormat(monthViewSettings.dayFormat, locale).format(currentDate).toUpperCase();
      ...
      _dayTextPainter.paint(
          canvas, Offset(xPosition + (width / 2 - _dayTextPainter.width / 2), yPosition));
    ...
    if (weekNumberPanelWidth != 0 && showWeekNumber) {
      ...
      _updateDayTextPainter(
          weekNumberTextStyle, weekNumberPanelWidth, localizations.weeknumberLabel);
      ...
      _dayTextPainter.paint(canvas,
          Offset(xPosition + (weekNumberPanelWidth / 2 - _dayTextPainter.width / 2), yPosition));
    }
  }
  ...
  void _addViewHeaderForTimeSlotViews(Canvas canvas, Size size, TextStyle viewHeaderDayStyle,
    ...
    for (int i = 0; i < visibleDatesLength; i++) {
      ...
      if (viewHeaderNotifier.value != null) {
        _addMouseHoverForTimeSlotView(canvas, size, xPosition, yPosition, dateXPosition, topPadding,
            isToday, inBetweenPadding);
      }

      _dateTextPainter.paint(
          canvas,
          Offset(xPosition + dateXPosition,
      ...
      if (!isDayView &&
        ...
        linePainter.color =
            weekNumberStyle.backgroundColor ?? calendarTheme.weekNumberBackgroundColor!;
        ...
        _dayTextPainter.paint(canvas,
            Offset(xPosition + (timeLabelWidth / 2 - _dayTextPainter.width / 2), yPosition));
      }
  ...
  void _addMouseHoverForMonth(
      Canvas canvas, Size size, double xPosition, double yPosition, double width) {
    if (xPosition + (width / 2 - _dayTextPainter.width / 2) <= viewHeaderNotifier.value!.dx &&
      ...
      _drawTodayCircle(
          ...
          hoveringColor: (themeData.brightness == Brightness.dark ? Colors.white : Colors.black87)
  ...
  void _drawTodayCircle(Canvas canvas, double x, double y, TextPainter dateTextPainter,
    ...
    canvas.drawCircle(
        Offset(x + painterWidth, y + painterHeight), radius + circlePadding, _circlePainter);
  }
  ...
  List<CustomPainterSemantics> _getSemanticsForDayHeader(Size size) {
    ...
    final double cellWidth =
        isDayView ? size.width : (size.width - timeLabelWidth) / visibleDates.length;
    if (isRTL) {
...
class _SelectionPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (selectedDate != null &&
        ((isMonthView &&
                !isDateWithInDateRange(calendar.minDate, calendar.maxDate, selectedDate)) ||
            (!isMonthView &&
                !CalendarViewHelper.isDateTimeWithInDateTimeRange(
  ...
  void _drawTimelineMonthSelection(Canvas canvas, Size size, double width) {
    ...
    for (int i = 0; i < visibleDates.length; i++) {
      if (isSameDate(visibleDates[i], selectedDate)) {
        ...
        final double height =
            selectedResourceIndex == -1 ? size.height : _yPosition + resourceItemHeight!;
  ...
  void _drawTimeLabels(
    ...
    for (int i = isTimelineView ? 0 : 1;
      ...
      if (rulerStyle != null && rulerStyle.backgroundColor != Colors.transparent) {
        final Rect backgroundRect = isTimelineView
            ? Rect.fromLTRB(
                xPosition,
                0,
                isRTL ? xPosition - timeIntervalHeight : xPosition + timeIntervalHeight,
                size.height)
...
class _CurrentTimeIndicator extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double minuteHeight =
        timeIntervalSize / CalendarViewHelper.getTimeInterval(timeSlotViewSettings);
...
class _ResizingAppointmentPainter extends CustomPainter {
  ...
  void _drawTimeIndicator(Canvas canvas, bool isTimelineView, Size size, bool isBackwardResize) {
    ...
    if (!isTimelineView &&
        resizingDetails.value.position.value!.dy < viewHeaderHeight + allDayHeight) {
      return;
    }
  ...
  void _addSubjectTextForTimeslotViews(Canvas canvas, int textStartPadding, double xPosition,
    ...
    double maxTextWidth =
        resizingDetails.value.appointmentView!.appointmentRect!.width - textStartPadding;
    ...
    if (isRecurrenceAppointment ||
        resizingDetails.value.appointmentView!.appointment!.recurrenceId != null) {
  ...
  void _addSubjectTextForMonthView(Canvas canvas, RRect appointmentRect, TextStyle style,
      TextSpan span, bool isRecurrenceAppointment, double xPosition, Rect rect, double yPosition) {
    double textSize = -1;
    if (textSize == -1) {
      ...
      for (double j = style.fontSize! - 1; j > 0; j--) {
        ...
        if (_textPainter.height >= appointmentRect.height) {
          ...
          span = TextSpan(
              text: resizingDetails.value.appointmentView!.appointment!.subject, style: style);
          _updateTextPainter(span);
        } else {
          textSize = j + 1;
          break;
  ...
  void _addRecurrenceIcon(
    ...
    if (view == CalendarView.month) {
      ...
      double maxTextWidth =
          resizingDetails.value.appointmentView!.appointmentRect!.width - textPadding! - 2;
      maxTextWidth = maxTextWidth > 0 ? maxTextWidth : 0;
      _textPainter.layout(maxWidth: maxTextWidth);
      canvas.drawRRect(
          RRect.fromRectAndRadius(
